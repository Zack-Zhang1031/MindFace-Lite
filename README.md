...
| GRID 数据 | 支持 GRID 音频预处理、伪标签训练、视频 landmark 标签提取入口 | 真实数据扫描、manifest、特征/标签落盘 |
| 推理部署 | PyTorch 推理、ONNX 导出、ONNXRuntime 推理、benchmark | 部署链路、性能统计、模型输入输出一致性 |
...
- 真正的视频口型监督训练需要先完成 GRID 视频 landmark / blendshape 标签提取和音视频标签对齐。
...
-> 实时队列模拟 -> benchmark -> ONNX INT8 动态量化 -> 量化 benchmark
```
