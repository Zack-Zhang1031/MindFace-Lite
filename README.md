...
- 统一 CLI、方向键教学工作台、pytest 测试入口、PyTorch vs ONNXRuntime vs RKNN 可选一致性报告已实现。
...
| 功能范围 | 推荐环境 | 说明 |
| --- | --- | --- |
| demo、data、train、infer、optimize | Windows `mindface-lite` | 基础演示、训练、ONNX、量化、剪枝、benchmark |
| GRID landmark | Windows `mindface-lite` | MediaPipe Windows 环境已验证 |
| realtime、tts | Windows `mindface-lite` | 麦克风和 OpenCV 窗口预览更稳定 |
| deploy rknn | WSL `mindface-rknn` | RKNN-Toolkit2 转换环境 |
| deploy device-tree | WSL `mindface-rknn` | Device Tree Compiler 和 ARM64 交叉编译 |
...
首次安装、还不能运行 `mindface` 短命令时，可以直接从源码启动工作台；环境安装模块只依赖 Python 标准库：
...
如果已经能进入 `mindface-lite`，推荐使用已安装的短命令检查或修复两个环境：
...
- 快速查看 Windows、WSL、Conda 和 RKNN venv 状态。
- 完整检查 CUDA/GPU、Python 包、RKNN、CMake、dtc 和 ARM64 编译器。
- 创建或修复 Windows `mindface-lite`，不会删除已有环境。
- 通过 WSL2 `Ubuntu` 创建或修复 `~/.venvs/mindface-rknn`。
- 每次选择官方、清华或阿里云 Python 下载源。
- 先查看 dry-run 计划，再一次确认并自动执行。
...
安装日志分别写入 `outputs/logs/environment-install-windows.log` 和 `outputs/logs/environment-install-wsl.log`。
...
菜单按“环境与安装、快速体验、数据处理、模型训练、推理部署、实时 TTS、C++ 边缘部署、项目检查”分组。所有功能都会显示，当前环境不能运行的项目会标明 Windows、WSL RKNN、Linux BSP 或硬件要求。配置型功能默认使用常用 YAML，也可以在执行前选择自定义 YAML。
...
mindface config show configs/training/train-mlp.yaml
...
训练会同时写入最佳 checkpoint 和每 epoch 更新的 `.last.pt`。`.last.pt` 包含模型、optimizer、epoch、FeatureSpec 和训练元数据；把 `train.resume_from` 指向该文件即可继续训练。
...
mindface realtime microphone --list-devices
mindface realtime microphone --config configs/realtime/mic-stream.yaml --duration-sec 10 --show
```

如果 `--list-devices` 能看到多个输入设备，可以在 `configs/realtime/mic-stream.yaml` 中设置：
...
C++ 公共库位于 `cpp/include/mindface` 与 `cpp/src`，应用位于 `cpp/apps`，队列测试位于 `cpp/tests`。
...
```text
MindFace-Lite/
├── configs/                 # 按功能域分组的 YAML 配置
│   ├── demos/               # 规则与视觉 demo
│   ├── datasets/            # 合成数据和 GRID 预处理
│   ├── training/            # MLP / LSTM / TCN / Transformer 训练
│   ├── inference/           # PyTorch / ONNXRuntime 推理
│   ├── optimization/        # 量化和剪枝
│   ├── benchmarks/          # 性能与一致性比较
│   ├── realtime/            # 队列、TTS、麦克风
│   └── deployment/          # ONNX、RKNN、Device Tree
├── assets/                  # 生成或准备的演示图片资产
├── data/                    # 原始数据和处理后数据
├── docs/                    # 分阶段文档、调试、面试说明
│   ├── requirements/        # 需求文档
...
├── models/external/         # 下载的第三方模型，不放训练生成模型
├── scripts/                 # 旧编号命令的兼容跳转入口，不含业务逻辑
├── src/mindface/            # Python 包源码
│   ├── audio/               # 音频读取和特征
│   ├── artifacts/           # 版本化 ModelBundle
│   ├── backends/            # PyTorch / ONNXRuntime / RKNN 接口
│   ├── configuration/       # YAML schema 校验
│   ├── commands/            # CLI 内部命令处理层
│   ├── data/                # 数据集和 GRID 处理
│   ├── diagnostics/         # 健康检查
│   ├── deploy/              # ONNX / RKNN 工具
│   ├── experiments/         # 实验追踪
│   ├── models/              # MLP / LSTM / TCN / Transformer
│   ├── pipelines/           # 可选择、可干跑的阶段 Pipeline
│   ├── realtime/            # 实时队列和麦克风
│   ├── tts/                 # 伪 TTS 和真实 TTS
│   ├── terminal_ui.py       # 方向键教学工作台
│   ├── utils/               # 配置、日志、CSV
│   └── visual/              # 嘴巴动画绘制
├── cpp/                     # C++ 实时控制 demo
├── tests/                   # pytest 自动测试
