...
```powershell
python scripts/14_extract_grid_video_landmarks.py --config configs/datasets/grid-video-landmarks.yaml --quality-only
```
...
```powershell
python scripts/22_real_tts_mouth_demo.py --config configs/realtime/real-tts.yaml
```
...
如果看到 `Error querying device -1` 或 `default.device=[-1, -1]`，说明 PortAudio 没有看到默认输入设备。优先在 Windows `mindface-lite` 环境运行麦克风 streaming；如果 `--list-devices` 能列出输入设备，再把对应编号或名称写入 `configs/realtime/mic-stream.yaml` 的 `audio.device`。

运行 10 秒：

```powershell
python scripts/23_mic_stream_rule_demo.py --config configs/realtime/mic-stream.yaml --duration-sec 10 --show
```
...
cd /mnt/c/Users/Administrator/Desktop/MindFace-Lite
source ~/.venvs/mindface-rknn/bin/activate
python scripts/24_rknn_convert_and_infer.py --config configs/deployment/rknn-deploy.yaml --quantize
```
...
- RKNN 不接受 ONNX 动态输入 `['frames', 70]`。项目会在 `rknn.load_onnx()` 中固定输入 shape，默认来自 `configs/deployment/rknn-deploy.yaml`：
