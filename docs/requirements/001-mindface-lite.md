...
- 作为环境维护者，我希望从 Windows 菜单检查、创建或修复 Windows 与 WSL 环境，不需要手工拼接整套安装命令。
...
- 必须提供 `mindface env status/check/install-windows/install-wsl`，覆盖 Windows Conda 与 WSL2 Ubuntu RKNN 环境。
- Windows 安装必须使用 `mindface-lite`、Python 3.10，并安装 core、optional、dev 和 editable package。
- WSL 安装必须使用 `~/.venvs/mindface-rknn`，只安装 RKNN requirements，不得混入 Windows optional requirements。
- 环境安装必须支持官方、清华、阿里云 pip 源逐次选择、dry-run、一次确认、执行日志和失败停止。
- 已有环境必须复用和修复，安装器不得自动删除环境。
...
- 环境安装失败后必须保留之前成功的步骤，并允许用户修复网络或依赖后重新运行。
...
- 环境矩阵报告：生成时间、检查模式、pass/warn/fail 汇总、Windows/WSL 检查项及详情。
- 环境安装计划：目标环境、项目根目录、按顺序执行的步骤名称、说明和参数数组。
- 环境安装日志：目标、时间、每一步命令和退出码。
...
环境安装项必须提供“开始安装”“仅查看安装计划”“返回”，并在每次安装时选择下载源。选择“开始安装”作为唯一一次确认，之后自动执行；`sudo` 密码仍由 WSL 终端安全输入。
...
- 使用环境计划单元测试保证不包含删除命令、WSL 不安装 optional requirements、下载源不写入全局配置。
- 使用伪 runner 验证环境安装任一步失败后停止并生成日志。
- 使用实际 `env status/check` 验证 Windows Conda、CUDA、WSL Ubuntu、RKNN、dtc 和交叉编译器探测。
